...
def _patch_app_handle(app: web.Application) -> None:
    if getattr(app, "_bt_css_patched", False):
        _LOGGER.warning("Btechnics IOT: patch al actief, skip")
        return
    ...
    async def _patched_handle(request: web.Request) -> web.StreamResponse:
        ...
        if path == "/manifest.json":
            try:
                ...
                if raw:
                    ...
                    _LOGGER.warning("Btechnics IOT: manifest.json gepatcht")
                    return web.Response(
                        text=json.dumps(manifest), status=200,
                        content_type="application/manifest+json",
                    )
                else:
                    _LOGGER.warning("Btechnics IOT: manifest body niet gevonden, type=%s", type(response).__name__)
            except Exception as err:
                _LOGGER.warning("Btechnics IOT manifest fout: %s", err)
        ...
        try:
            ...
            # FileResponse: lees bestand van disk
            if hasattr(response, "_path") and response._path:
                try:
                    text = pathlib.Path(str(response._path)).read_text(encoding="utf-8")
                    _LOGGER.warning("Btechnics IOT: FileResponse gelezen van %s", response._path)
                except Exception as ferr:
                    _LOGGER.warning("Btechnics IOT: FileResponse leesfout: %s", ferr)

            # Gewone Response: lees body
            if text is None:
            ...
            if text and "<head>" in text and "bt-hide" not in text:
                ...
                _LOGGER.warning("Btechnics IOT: CSS geinjecteerd (type=%s)", resp_type)
                return web.Response(
                    text=patched, status=response.status,
                    content_type="text/html", charset="utf-8",
                )
            elif text is None:
                _LOGGER.warning("Btechnics IOT: body is None, type=%s path=%s", resp_type, path)
            elif "bt-hide" in text:
                _LOGGER.warning("Btechnics IOT: CSS al aanwezig")
        except Exception as err:
            _LOGGER.warning("Btechnics IOT CSS fout: %s", err)

        return response

    app._handle = _patched_handle
    app._bt_css_patched = True
    _LOGGER.warning("Btechnics IOT: _handle wrapper ACTIEF")
...
async def async_setup_entry(hass: HomeAssistant, entry) -> bool:
    _LOGGER.warning("Btechnics IOT: async_setup_entry start")
    try:
    ...
    _LOGGER.warning("Btechnics IOT: setup_entry volledig")
    return True


async def async_update_listener(hass: HomeAssistant, entry) -> None:
    _LOGGER.warning("Btechnics IOT: instellingen bijgewerkt")
